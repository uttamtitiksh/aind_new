...
# -*- coding: utf-8 -*-
"""
Created on Fri May 11 05:36:11 2018
...
def play_matches(cpu_agents, test_agents, num_matches):
    ...
    print("\n{:^9}{:^13}".format("Match #", "Opponent") + ''.join(['{:^21}'.format(x[1].name) for x in enumerate(test_agents)]))
        #for row in newFileWriter:
        #newFileWriter.writerow("Match #", "Opponent","AB_Improved","AB_Custom","AB_Custom_2","AB_Custom_3")      
        #print("{:^9}{:^13} ".format("", "") +  ' '.join(['{:^5}| {:^5}'.format("Won", "Lost") for x in enumerate(test_agents)]))
    print("{:^9}{:^13} ".format("", "") +  ' '.join(['{:^5}| {:^5}| {:^5}'.format("Won", "Lost", "Rate") for x in enumerate(test_agents)]))
    for idx, agent in enumerate(cpu_agents):
        ...
        print(' ' + ' '.join([
            '{:^5}| {:^5}| {:^5}'.format(
                round_totals[i],round_totals[i+1],"{:.0f}%".format((round_totals[i]/(round_totals[i]+round_totals[i+1]))*100)
